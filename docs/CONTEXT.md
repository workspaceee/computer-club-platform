...
| Пункт | Модель | Почему |
|---|---|---|
| `C2.13` | СИЛЬНАЯ | закрытая дверь входа офлайн — контракт отказов в транспорте; **ТЗ уже готово** (`tasks/C2.13.md`) |
...
**Открыт хвост `C2`: `C2.13`–`C2.19`** — офлайн (закрытая дверь входа + двухступенчатый учёт
времени). Заведён из [`OFFLINE-TIME.md`](./OFFLINE-TIME.md); порядок `C2.14 → C2.15 → C2.16 →
C2.17 → C2.18` обязателен, `C2.13` независим и у него **уже есть ТЗ** (`tasks/C2.13.md`).
...
**Следующий пункт: `C2.13`**, затем `C2.14`. `C4.1` (библиотека: сетка обложек, реальные
сгенерированные обложки вместо градиентов, `GameCover` с ленивой загрузкой) — после хвоста `C2`
