...
**Открыт хвост `C2`: `C2.18`–`C2.19`** — двухступенчатый учёт времени. `C2.13` (закрытая дверь
...
вне `OFFLINE_BLOCKED`. Остальное заведено из [`OFFLINE-TIME.md`](./OFFLINE-TIME.md); порядок
`C2.18 → C2.19` обязателен.
...
**Следующий пункт: `C2.18`** (показание не верит системным часам: elapsed для отчёта — монотонно,
skew-коррекция на экране остаётся). `C4.1` (библиотека: сетка обложек, реальные
...
Что сломано в учёте времени **прямо сейчас** (лечится `C2.18`): elapsed для отчёта мерится
разностью системных часов, а не монотонно, — переведённые на киоске часы двигают показание.
Периодический отчёт и догон на реконнекте закрыты в `C2.15`/`C2.16`, неидемпотентность — в `C2.14`
(сервер берёт `max` по показанию), офлайн-потолок postpaid — в `C2.17`.
