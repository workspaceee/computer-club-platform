...
**Открыт хвост `C2`: `C2.15`–`C2.19`** — двухступенчатый учёт времени. `C2.13` (закрытая дверь
...
Остальное заведено из [`OFFLINE-TIME.md`](./OFFLINE-TIME.md); порядок `C2.15 → C2.16 →
...
**Следующий пункт: `C2.15`.** `C4.1` (библиотека: сетка обложек, реальные
...
Что сломано в учёте времени **прямо сейчас** (лечится `C2.15`–`C2.16`): `heartbeat` никто не
вызывает по таймеру (единственный вызов — `lib/seat.ts`), ошибка отчёта проглатывается
(`.catch(() => {})` там же). Неидемпотентность закрыта в `C2.14` — сервер берёт `max` по показанию.
