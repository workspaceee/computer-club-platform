...
**`C3.1`–`C3.12`**. Всего 110 принятых пунктов. Долг нумерации секций на гостевой поверхности
закрыт в `C3.11` (`useSectionIndices` в `home-view.tsx`).

**Следующий пункт: `C3.13`** — пустой профиль нового игрока: домашняя не выглядит мёртвой,
вместо пустых карточек — «Выполни первый квест», «Выбери первую игру», «Получи 30 минут за
первого друга».
...
их надо либо объявить в §13, либо удалить.

Дальше: `C3.13` (пустой профиль), затем `C4` (игры) → `C15` (приёмка Этапа 1).
