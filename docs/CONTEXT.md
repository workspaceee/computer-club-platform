...
**Открыт хвост `C2`: `C2.14`–`C2.19`** — двухступенчатый учёт времени. `C2.13` (закрытая дверь
...
Остальное заведено из [`OFFLINE-TIME.md`](./OFFLINE-TIME.md); порядок `C2.14 → C2.15 → C2.16 →
C2.17 → C2.18` обязателен.
...
**Следующий пункт: `C2.14`.** `C4.1` (библиотека: сетка обложек, реальные
...
Что сломано в учёте времени **прямо сейчас** (лечится `C2.14`–`C2.16`): `heartbeat` никто не
вызывает по таймеру (единственный вызов — `lib/seat.ts:160`), ошибка отчёта проглатывается
(`.catch(() => {})` там же), и сам `heartbeat` неидемпотентен (`+=` в `lib/mock/api/session.ts:245`).
...
их надо либо объявить в §13, либо удалить; `tsc --noEmit` падает в `first-run-tour.tsx:366` (тип
пропа `t` у `TourCard` шире `TKey`) — долг `C3.12`, блокирует `next build`.
