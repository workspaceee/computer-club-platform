 ...
 *
 * Размеры — из шкалы `ICON_SIZE` (§12: 13/16/20/24), крупные плиты пустых
 * состояний рисует `IconTile`, а не `size={56}` руками.
 */
import {
  Activity,
  AlertTriangle,
...
/**
 * Шкала кеглей (§12). `xs` — чипы телеметрии и микро-бейджи, `sm` — иконка в
 * поле и в строке списка, `md` — кнопка и шапка модалки, `lg` — заголовок
 * раздела. Всё, что крупнее, — плита `IconTile`, а не иконка.
 */
export const ICON_SIZE = { xs: 13, sm: 16, md: 20, lg: 24 } as const
