...
function StatCard({
  ...
  return (
    <motion.div
      ...
      transition={{ delay }}
      whileHover={{ y: -3 }}
      className="group relative overflow-hidden rounded-xl border border-border bg-surface px-4 py-4"
    >
      ...
      <p className="mt-3 font-display text-2xl font-black leading-none tracking-tight text-text-high">{value}</p>
      <p className="mt-1.5 text-xs font-medium uppercase tracking-wide text-text-low">{label}</p>
    </motion.div>
  )
}
...
function XpRing({ pct, initials, level }: { pct: number; initials: string; level: number }) {
  ...
  return (
    <div className="relative h-28 w-28 shrink-0">
      ...
      <div className="absolute inset-2.5 flex items-center justify-center rounded-full border border-primary/30 bg-primary/10 font-display text-2xl font-black text-primary">
        {initials}
      </div>
      <span className="absolute -bottom-1 left-1/2 flex -translate-x-1/2 items-center gap-1 rounded-full border border-primary/40 bg-surface-2 px-2.5 py-0.5 font-display text-[11px] font-bold text-text-high shadow-lg">
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-6 px-4 py-6 md:px-6">
      ...
      >
        ...
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(229,53,43,0.22),transparent_60%)]" />
        ...
        <div className="relative flex flex-col gap-6 p-6 md:flex-row md:items-center md:gap-8">
          ...
          <div className="flex-1">
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="font-display text-3xl font-black tracking-tight text-text-high">{user.nickname}</h1>
              <span className="rounded-full border border-success/30 bg-success/15 px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wide text-success">
                Online
              </span>
            </div>
            ...
            <div className="mt-4 max-w-md">
              <div className="mb-1.5 flex items-center justify-between font-display text-xs font-semibold uppercase tracking-wide text-text-medium">
                <span>
          ...
          <div className="flex flex-row gap-3 md:flex-col">
            <div className="flex flex-1 items-center gap-2.5 rounded-xl border border-warning/25 bg-warning/10 px-4 py-3">
              <Coins className="h-5 w-5 text-warning" />
              <div>
                <p className="font-display text-lg font-black leading-none text-text-high">{coins.toLocaleString()}</p>
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 rounded-xl border border-border bg-surface px-4 py-3 text-sm font-semibold text-text-high transition-colors hover:border-border-strong hover:bg-white/5"
            >
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        ...
        <section className="lg:col-span-3">
          <div className="mb-3 flex items-end justify-between gap-4">
            <h2 className="font-display text-sm font-black uppercase tracking-wider text-text-high">Achievements</h2>
            ...
            {ACHIEVEMENTS.map((a, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`group relative flex flex-col items-center gap-2 overflow-hidden rounded-xl border px-3 py-4 text-center transition-all ${
                    a.unlocked
        ...
        <section className="lg:col-span-2">
          <h2 className="mb-3 font-display text-sm font-black uppercase tracking-wider text-text-high">
            Recent activity
          </h2>
          <ol className="relative flex flex-col">
            ...
            {ACTIVITY.map((e, i) => {
              ...
              return (
                ...
                >
                  <span
                    className={`relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-border bg-surface-2 ${
