    ...
    <section aria-labelledby="tournament-heading">
      ...
      <div className="glass tick-corners rounded-xl p-4">
        <DataBoundary
          ...
          errorSize="sm"
          loading={
            // The final height of the art and of the right-hand column, so the
            // surface does not resize when the bracket lands (C3.11).
            <div className="grid gap-4 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
              <Skeleton className="h-[168px] w-full" radius="md" />
              <div className="flex flex-col gap-3">
                <Skeleton className="h-[52px] w-40" />
                <Skeleton className="h-[76px] w-full" radius="md" />
                <Skeleton className="h-8 w-32" radius="md" />
              </div>
            </div>
          }
          ...
          {(data) => {
            ...
            return (
              <motion.div
                ...
                className="grid gap-4 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]"
              >
                {/* ── What is on, and on what ───────────────────────────── */}
                <div className="relative h-[168px] overflow-hidden rounded-md border border-border">
                  {data.game ? (
                    <>
                  ...
                  <div className="absolute inset-x-0 bottom-0 flex flex-col gap-1 p-3">
                    ...
                    <p className="text-balance font-display text-base font-bold uppercase leading-tight tracking-tight text-text-high">
                      {tournament.name}
                    </p>
                ...
                {/* ── When, what it costs, and the one thing to press ───── */}
                <div className="flex flex-col gap-3">
                  <StartsIn startsAt={tournament.startsAt} />

                  <div className="grid grid-cols-2 gap-2">
                    {/* No label: the club's own plural already contains the noun
                        ("3 of 16 slots left"), and a "Slots" caption above it
                    ...
                    <Fact
                      ...
                      tone={tournament.slotsFree > 0 ? undefined : 'muted'}
                    />
                    <Fact
                      icon={icons.timer}
                      label={t('home.tournamentFormat')}
                      value={t(FORMAT_KEY[tournament.format])}
                    />
                    {tournament.prizes[0] && (
                      <Fact
                        icon={icons.gift}
                        label={t('home.tournamentPrize')}
                        // The club promises the prize in its own words.
                        value={tournament.prizes[0].label}
                      />
                    )}
                    <Fact
                      icon={paid ? icons.payment : icons.check}
                      label={t('home.tournamentEntry')}
                      value={fee}
                      tone={paid ? undefined : 'success'}
                    />
                  </div>
 ...
 */
function StartsIn({ startsAt }: { startsAt: ISODateTime }) {
  ...
  const view = useMemo(() => {
    ...
    if (seconds <= 0) return { display: null, spoken: t('home.tournamentStartingNow') }
    ...
    if (seconds >= SECONDS_PER_DAY) {
      ...
      return { display: label, spoken: `${t('home.tournamentStartsIn')} ${label}` }
    }
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {/* `font-clock`, but no neon: the one running ring on this screen belongs to
          the session plate (§4.2). */}
      {view.display ? (
        <span
          aria-hidden
          className="font-clock text-4xl font-semibold leading-none tabular-nums text-text-high"
        >
          {view.display}
        </span>
      ) : (
      ...
      {/* A duration is not a plan; a time of day is. 24-hour in all three
          locales (F3.7). */}
...
/**
 * One fact about the bracket: slots, format, prize, entry.
 *
 * Recessed into the panel — the shallow rung of the well family (§3.3). Facts are
 * promises; the element allowed to step forward on this card is the button.
 */
function Fact({
  icon: Icon,
  label,
  ...
  icon: LucideIcon
  /** Omitted when the value already contains its noun ("3 of 16 slots left"). */
  label?: string
  value: string
  tone?: 'success' | 'muted'
}) {
  return (
    <div className="well-shallow flex min-w-0 items-start gap-2 rounded-md border border-border px-3 py-2">
      <Icon size={14} aria-hidden className="mt-0.5 shrink-0 text-text-low" />
      <div className="flex min-w-0 flex-col gap-0.5">
        {label && <p className="label-mono text-[9px] text-text-low">{label}</p>}
        <p
          className={cn(
            'text-pretty text-xs font-semibold leading-snug',
            tone === 'success' ? 'text-success' : tone === 'muted' ? 'text-text-low' : 'text-text-high',
          )}
        >
          {value}
        </p>
